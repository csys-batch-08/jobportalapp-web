...
public List<PostJobModel> viewJobs(String email) {	 
	 ...
	 ResultSet rs=null;
	 try {
	 String query ="select p.company_id,p.post_id,p.job_title,p.salary,p.experience,p.category,p.post_date,c.email from posting_job p,"
	 		      + "Company_login c where p.company_id=c.company_id and c.email=? order by post_date desc ";
...
public List<PostJobModel> SearchJobs(String Location) {
	 ...
	 try {
	  String query =  "select p.company_id,c.company_name,p.post_id,p.salary,p.job_title,c.location,p.experience  "
	 	        	+ "from posting_job p ,Company_login c\r\n"
...
public List<PostJobModel> SearchExp(String Experience)  {
    ...
    ResultSet rs=null;
	 try {
	 String query =  "select p.company_id,c.company_name,p.post_id,p.salary,p.job_title,c.location,p.experience  from posting_job p"
	 		+ " ,Company_login c\r\n"
