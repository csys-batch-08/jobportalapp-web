...
<body>
	<table border="2" id="alljobs" class="table">
		<h1 align="center">
			<strong>Search By Experience List</strong>
		</h1>
		<thead>
			<tr>
				<th>S.No</th>
		...
		<tbody>
			<c:forEach items="${Experience}" var="current">
				...
				<tr>
					...
					<td class="table-warning"><c:out
							value="${current.getIncome()}" /></td>
					<td class="table-active"><c:out
							value="${current.getJobTitle()}" /></td>
					<td class="table-secondary"><c:out
							value="${current.getLocation()}" /></td>
					<td class="table-warning"><c:out
							value="${current.getService()}" /></td>
					<td class="table-light"><a
						href="applyJob.jsp?postid=<c:out value="${current.getPostId()}&postId=${current.getPostId()}" />">Apply</a></td>
